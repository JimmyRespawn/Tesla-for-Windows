...
try {
    ...
    Add-Type -Path @($httpPath, (Join-Path $PSScriptRoot 'DriverManagementChecks.cs'),
        (Join-Path $repoPath 'TeslaMurphy/Services/DriverManagementService.cs')) -ReferencedAssemblies $refs -CompilerOptions '/nowarn:1701'
    [TeslaMurphy.Services.DriverChecks]::Run().GetAwaiter().GetResult()
}
